...
class Port:
    ...
    @property
    def label(self) -> str:
        # Страна есть не у всякой точки: координаты, введённые руками, и
        # позиция с устройства -- без неё. Хвост из запятой в подписи
        # выглядит опечаткой, поэтому его не оставляем.
